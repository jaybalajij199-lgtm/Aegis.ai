...
// Replace the District Selector Tabs
const startIdx = content.indexOf('{/* District Selector Tabs */}');
const endStr = '      </div>\n\n      {/* Primary Weather Telemetry Cards */}';
const endIdx = content.indexOf(endStr, startIdx) + '      </div>'.length;
  ...
  const replacement = `{/* Location Search Navigation */}
      <div className="flex flex-col md:flex-row items-center gap-3">
        <div className="relative flex-1 w-full">
          <input
            type="text"
            placeholder="Search city, region, or address (or click map)..."
            value={searchQuery}
